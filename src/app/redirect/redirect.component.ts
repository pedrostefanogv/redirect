  ...
  template: `
    <div class="redirect-container">
      <div class="redirect-card">
        ...
        <div class="timer-container" *ngIf="delaySeconds > 0 && !isCanceled">
          <div class="timer-circle">
            ...
            <div class="timer-text">
              {{ isCanceled ? "Cancelado" : countdown }}
            </div>
          </div>
        </div>

        <p class="timer-label" *ngIf="!isCanceled && delaySeconds > 0">
          segundos restantes
        </p>
        ...
        <div class="github-link">
          <a
            href="https://github.com/pedrostefanogv/redirect"
            target="_blank"
...
export class RedirectComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    const paramsSub = this.route.queryParams
      .pipe(take(1))
      .subscribe((params: Record<string, string>) => {
        const raw = params["url"] || params["deeplink"] || "";
        this.delaySeconds = this.parseDelay(params["delay"]);
        this.targetUrl = this.normalizeUrl(raw);
        this.loading = false;
        this.totalDuration = Math.max(0, this.delaySeconds);

        if (this.targetUrl) {
          this.displayUrl = this.truncateUrl(this.targetUrl);
          if (this.totalDuration > 0) {
            this.countdown = this.totalDuration;
            this.updateProgress();
            this.startCountdown();
          } else {
            this.countdown = 0;
  ...
  private redirect(): void {
    if (!this.targetUrl || this.isCanceled || this.redirectStarted) return;

    this.redirectStarted = true;

    try {
      if (
        this.targetUrl.includes("://") &&
        !this.targetUrl.toLowerCase().startsWith("http")
      ) {
